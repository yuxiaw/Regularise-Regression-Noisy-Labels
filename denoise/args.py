...
def default_arguments(arguments):
    ...
    args = Namespace(
        ...
        do_filter=True if arguments[24] == "True" else False,
        do_repair=True if arguments[25] == "True" else False,
        do_resample=True if arguments[26] == "True" else False,
        gold_pred_gap=arguments[27],
        logfile_dir=arguments[28],
        noise_detect_criterion=arguments[29],
        weight_decay=arguments[30],
        device=arguments[31],
        n_gpu=int(arguments[32]),
    )
    return args
...
def parse_arguments(argv):
    ...
    parser.add_argument("--do_filter",
                        action='store_true',
                        help="Whether to filter noise labels iteratively during training.")
    parser.add_argument("--do_repair",
                        action='store_true',
                        help="Whether to repair noise labels iteratively during training.")
    parser.add_argument("--do_resample",
                        action='store_true',
                        help="Whether to discard noise labels and resample cases iteratively during training.")
    parser.add_argument("--gold_pred_gap",
                        default=0.75,
                        type=float,
                        help="Threhold set to distinguish noisy labels, the gap betweem gold labels and predictiosn.")
    parser.add_argument("--logfile_dir",
                        default="",
                        type=str,
                        help="Where do you want to store the training log")
    parser.add_argument("--noise_detect_criterion",
                        default="prediction_pearson",
                        type=str,
                        help="The criterion used to detect noisy labels in train data")
