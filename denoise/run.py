...
# author: yuxia wang
# date: July 1, 2020
# update data: Nov 21, 2020
# update data: Jan 6, 2021
"""BERT finetuning for STS task. 
   update: Noise filtering is incorporated.
   update: Add pearson metric criterion to noise detection"""
...
from util_noise import mse_loss, calculate_metrics, update_train_data
...
def load_inference_examples(args, tokenizer, data_file, evaluate=True, has_label=True):
    ...
    # Prepare data loader
    if has_label:
        examples = processor.get_train_examples(data_file) 
    else:
        examples = processor.get_infer_examples(data_file)  # all labels are given as 5.0
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    # cache features for train and dev sets
    ...
    if args.local_rank == -1:
        ...
        sampler = DistributedSampler(data)  # Note that this sampler samples randomly
    dataloader = DataLoader(data, sampler = sampler, batch_size = batch_size)
    # save vocab.txt, long-time no update leads to errors
...
def train(args):
    ...
    # infer_dataloader = load_inference_examples(args, tokenizer, 
    #                     os.path.join(args.data_dir, "dev_only.txt"), evaluate=True, has_label=True)
    dataloaders = {
    "train": train_dataloader,
    "validation": eval_dataloader
    }
    original_num_train_examples = len(dataloaders["train"].dataset.tensors[3])
    # dataloaders = {
    # "train": train_dataloader,
    # "validation": infer_dataloader
    # }
    ...
    for _ in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        # for phase in ['validation', 'train']:
            ...
            if phase == 'validation':
                ...
                if args.do_filter or args.do_repair or args.do_resample:
                    # save preds for each epoch
                    pred_dir = os.path.join(args.data_dir, "pred/")
                    # when train the first epoch, and pred_dir exists and not empty, empty it
                    if (_ == 0) and os.path.exists(pred_dir) and os.listdir(pred_dir):
                        for file in os.listdir(pred_dir):
                            os.remove(os.path.join(pred_dir, file))
                    # if pred_dir does not exist, mkdir
                    if not os.path.exists(pred_dir):
                        os.mkdir(pred_dir)
                    # each epoch initialise with original train data examples
                    # only save train data prediction
                    preds = np.squeeze(preds[0])[:original_num_train_examples] 
                    with open(os.path.join(pred_dir, "train_preds_{}_epoch.txt".format(_)), "w") as file:
                        preds = preds.round(decimals=2)
                        file.write("\n".join([str(j) for j in preds]))
                    # After noise filtering, repairing and resampling based on gold labels and preds
                    # save updated train.txt into savetraindir, in three different manners
                    # here, resample means after discarding noisy labels, 
                    # sample same number of cases from clean instances
                    savetraindir = os.path.join(args.data_dir, "epoch_{}/".format(_))
                    train_data_dir = args.data_dir
                    if args.do_filter:
                        update_train_data(pred_dir, train_data_dir, savetraindir, 
                        way_to_noise="discard", 
                        noise_detect_criterion = args.noise_detect_criterion, 
                        threshold=args.gold_pred_gap,
                    ...
                    if args.do_resample:
                        update_train_data(pred_dir, train_data_dir, savetraindir, 
                        way_to_noise="resample", 
                        noise_detect_criterion = args.noise_detect_criterion, 
                        threshold=args.gold_pred_gap,
                        voters = 5, num_clean_subset = 8, threshold_r = 0.01)
                    
                    # load updated train.txt from savetraindir, then assign to dataloader
                    train_dataloader = load_inference_examples(args, tokenizer, 
                    os.path.join(savetraindir, "train.txt"), evaluate=False, has_label=True)
    ...
    # evaluate(args, dataloader=infer_dataloader)
    output_args_file = os.path.join(args.output_dir, 'arguments.txt')
    save_args(args, output_args_file)
    tb_writer.close()

def evaluate(args, dataloader=None, save_preds=False):
    # Prepare data
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]
    ...
    with open(output_eval_file, "w") as writer:
        ...
        logger.info("avg_loss = %s", l)
        print("avg loss: %s" % str(l))
...
def inference(args_file, data_file=None, pred_file=None, savetext=False):
    """Predict semantic similarity scores for unlabelled sentence pairs
    args_file: txt file saving arguments or args variable
    data_file: the directory of data to predict, if none, default as train.txt in args.data_dir
    pred_file: the name of score prediction file to save, 
               if none, default is datafile[-7:-4]_pred.txt
    savetext: bool value, if true, save sentence pair text at the same time when saving scores"""
    # load arguments
    ...
    # save the predicted score and sentence pairs in predfilename
    pred_dir = os.path.join(args.data_dir, "pred/")
    if not os.path.exists(pred_dir):
        os.mkdir(pred_dir)
    if pred_file is None:
        predfilename = os.path.join(pred_dir, data_file[-7:-4]+"_preds.txt")
    else:
        predfilename = os.path.join(pred_dir, pred_file)
    with open(predfilename, "w") as file:
        preds = preds.round(decimals=2)
        if not savetext:
            file.write("\n".join([str(j) for j in preds]))
        else:
...
if __name__ == "__main__":
    # inference("./models/sota/arguments.txt", data_file="./data/mtd/syn/train.txt", 
    #           pred_file="train.txt", savetext=True)
    main()
